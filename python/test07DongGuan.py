...
def getContent():
    ...
    Browerdriver.get("http://edu.dg.gov.cn/jyzx/jyyw/")
    # 打印页面标题“百度一下你就知道”
    ...
    elementul =Browerdriver.find_element(By.XPATH, '//div[@class="con-right fr"]')  # element01Content=Browerdriver.find_element(By.XPATH,'//div[@class="moe-detail-box"]')
    ...
    for link in links:
        ...
        try:
            ...
            Out_Comfrom = "东莞教育"
            CurAticle = Browerdriver.find_element(By.XPATH, '//div[@id="cen"]')

            Out_titleRarticle1 = CurAticle.find_element(By.XPATH, '//div[@class="con text"]')
            Out_titleRarticle2 = Out_titleRarticle1.find_element(By.XPATH, '//ucaptitle')
            Out_title=Out_titleRarticle2.text



            element02Contentucapcontent = CurAticle.find_element(By.XPATH, '//div[@id="zoomcon"]')
            ...
            savePic(img_elements, "Pic241DongGuan", maxCount, Browerdriver)
            Out_Content = element02Content.get_attribute('innerHTML')
            saveContent(Out_title, Out_Content, Out_Comfrom, sourceContent, 241)
