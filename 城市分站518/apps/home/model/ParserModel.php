...
class ParserModel extends Model
{
    ...
    {
        return parent::table('ay_content_city')->where("filename='" .$city . "'")->find();
    }
		 // 调用城市分站 JUSRUSH
    public function getAllCity()
    {
        return parent::table('ay_content_city')->where("status='1'")->field('name,filename')->select();
    }
    ...
    public function getCitysTree()
    {
        ...
        $result = parent::table('ay_content_city a')->where("a.acode='" . get_lg() . "'")
            ->where('a.status=1')
            ->join($join)
            ->order('a.pcode,a.sorting,a.id')
            ->column($fields, 'scode');
