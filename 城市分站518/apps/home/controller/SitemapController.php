...
class SitemapController extends Controller
{
    ...
    public function index($city_file='')
    {
        ...
        $cityurl='';  
        if(!empty($city_file) && $this->config('sitemap_type')==0 ){
            $cityurl='/'.$city_file;
            $str .= $this->makeNode($cityurl.'/', date('Y-m-d'), '1.00', 'always');
        }else {
...
if($this->config('sitemap_close')==0 &&$this->config('sitemap_type')==1){
