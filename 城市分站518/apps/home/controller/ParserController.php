...
class ParserController extends Controller
{
    ...
    // 解析全局后置公共标签
    public function parserAfter($content,$city= null,$ishome=0)
    {
        // 默认页面信息替换
        ...
        $content = $this->parserSiteLabel($content,$city); // 站点标签
        $content = $this->parserCompanyLabel($content); // 公司标签
        $content = $this->parserMemberLabel($content); // 会员标签
        $content = $this->parserNavLabel($content,$city); // 分类列表
        $content = $this->parserCityLabel($content);
        $content = $this->parserSelectAllLabel($content); // CMS筛选全部标签解析
        $content = $this->parserSelectLabel($content); // CMS筛选标签解析
        $content = $this->parserSpecifySortLabel($content,$city); // 指定分类
        $content = $this->parserListLabel($content,'',$city);// 指定列表
        ...
        $content = $this->parserCity($content,$city,$ishome);
        return $content;
    }
...
// 解析站点标签
    public function parserCity($content,$city,$ishome)
	{
      if($city){     
        $data = $this->model->getCity($city);
        if($data){
           if($ishome==1){
           if($data->title){ $content = preg_replace('/<title>(.*)<\/title>/si', '<title>'.$data->title.'</title>', $content);}
           if($data->keywords){ $content = preg_replace('/<META\s+name=[\'|\"]keywords[\'|\"]\s+content=[\'|\"]([\w\W]*?)[\'|\"]/si', '<meta name="keywords" content="'.$data->keywords.'"', $content);} 
           if($data->description){ $content = preg_replace('/<META\s+name=[\'|\"]description[\'|\"]\s+content=[\'|\"]([\w\W]*?)[\'|\"]/si', '<meta name="description" content="'.$data->description.'"', $content);}           
        }
        if($data->name)
          $content = str_replace('{city}', $data->name, $content);
          $content = str_replace('{citynow}', $data->name, $content);
        }else {
        ...
        return $content;
          ...
          $content = str_replace('{city}','', $content); 
          $content = str_replace('{citynow}', '主站', $content);
          return $content;
      }	
	}  

    // 解析站点标签
    public function parserSiteLabel($content,$city)
    {
        ...
        if($city){
        $data = $this->model->getCity($city);  
        $content = str_replace('{pboot:sitepath}', '{pboot:sitepath}/'.$city, $content);
        if($data->gsname){$content = str_replace('{pboot:companyname}', $data->gsname, $content);}
        if($data->address){$content = str_replace('{pboot:companyaddress}', $data->address, $content);}  
        if($data->postcode){$content = str_replace('{pboot:companypostcode}', $data->postcode, $content); } 
        if($data->contact){$content = str_replace('{pboot:companycontact}', $data->contact, $content);}
        if($data->mobile){$content = str_replace('{pboot:companymobile}', $data->mobile, $content); } 
        if($data->phone){$content = str_replace('{pboot:companyphone}', $data->phone, $content); } 
        if($data->fax){$content = str_replace('{pboot:companyfax}', $data->fax, $content);  }
        if($data->email){$content = str_replace('{pboot:companyemail}', $data->email, $content); } 
        if($data->qq){$content = str_replace('{pboot:companyqq}', $data->qq, $content);   } 
        }
        // 页面自适应标题避免多横线
    ...
    // 解析栏目列表标签
    public function parserNavLabel($content,$city)
    {
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                            ...
                            switch ($matches2[1][$j]) {
                                ...
                                case 'link':
                                    if ($value['outlink']) {
                                        ...
                                        $one_html = str_replace($matches2[0][$j], $this->parserLink($value['type'], $value['urlname'], 'list', $value['scode'], $value['filename'], '', '',$city), $one_html);
                                    }
                                    break;
                                case 'soncount':
                ...
                if (preg_match($pattern3, $out_html, $matches3)) {
                    ...
                    $out_html = $this->parserNavLabel($out_html,$city);
                }
     ...
     // 解析城市列表标签
    public function parserCityLabel($content)
    {
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                if (preg_match($pattern3, $out_html, $matches3)) {
                    ...
                    $out_html = $this->parserCityLabel($out_html);
                }
    ...
    // 解析当前位置
    public function parserPositionLabel($content, $scode, $page = null, $link = null,$city= null)
    {
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                $out_html = '<a href="'.SITE_INDEX_DIR . '/'.$url.'">' . $indextext . '</a>';
                if ($page && $scode == 0) {
                    ...
                    foreach ($data as $key => $value) {
                        if ($value['outlink']) {
                            ...
                            $out_html .= $separator . '<a href="' . $this->parserLink($value['type'], $value['urlname'], 'list', $value['scode'], $value['filename'], '', '',$city) . '">' . $value['name'] . '</a>';
                        }
                    }
                }
                // 执行内容替换
    ...
    // 解析当前分类标签
    public function parserSortLabel($content, $sort,$city='')
    {
        $pattern = '/\{sort:([\w]+)(\s+[^}]+)?\}/';
        if (preg_match_all($pattern, $content, $matches)) {
            $count = count($matches[0]);
                ...
                switch ($matches[1][$i]) {
                    case 'link':
                        if ($sort->outlink) {
                            ...
                            $content = str_replace($matches[0][$i], $this->parserLink($sort->type, $sort->urlname, 'list', $sort->scode, $sort->filename, '', '',$city), $content);
                        }
                        break;
                    case 'tcode': // 顶级栏目ID
                        if (! isset($tcode))
                    ...
                    case 'toplink':
                        ...
                        if ($top_sort->outlink) {
                            ...
                            $toplink = $this->parserLink($top_sort->type, $top_sort->urlname, 'list', $top_sort->scode, $top_sort->filename, '', '',$city);
                        }
                    ...
                    case 'parentlink':
                        ...
                        if ($parent_sort->outlink) {
                            ...
                            $parentlink = $this->parserLink($parent_sort->type, $parent_sort->urlname, 'list', $parent_sort->scode, $parent_sort->filename, '', '',$city);
                        }
    ...
    // 解析非列表页分类标签
    public function parserSpecialPageSortLabel($content, $id, $page, $link,$city= null)
    {
        $pattern = '/\{sort:([\w]+)(\s+[^}]+)?\}/';
        if (preg_match_all($pattern, $content, $matches)) {
            $count = count($matches[0]);
    ...
    // 解析指定分类标签
    public function parserSpecifySortLabel($content,$city)
    {
    ...
    // 解析内容列表标签
    public function parserListLabel($content, $cscode = '',$city='')
    {
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                foreach ($data as $value) { // 按查询数据条数循环
                    ...
                    for ($j = 0; $j < $count2; $j ++) { // 循环替换数据
                        $params = $this->parserParam($matches2[2][$j]);
                        $one_html = $this->parserList($matches2[1][$j], $matches2[0][$j], $one_html, $value, $params, $key,$city);
                    }
                    $key ++;
                    $out_html .= $one_html;
                }
    ...
    // 解析当前内容标签
    public function parserCurrentContentLabel($content, $sort, $data,$city=null)
    {
        $pattern = '/\{content:([\w]+)(\s+[^}]+)?\}/';
        if (preg_match_all($pattern, $content, $matches)) {
            $count = count($matches[0]);
            for ($i = 0; $i < $count; $i ++) {
                ...
                $params = $this->parserParam($matches[2][$i]);
                $content = $this->parserContent($matches[1][$i], $matches[0][$i], $content, $data, $params, $sort,$city);
            }
        }
    ...
    public function parserSpecifyContentLabel($content)
    {
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                for ($j = 0; $j < $count2; $j ++) { // 循环替换数据
                    $params = $this->parserParam($matches2[2][$j]);
                    $out_html = $this->parserContent($matches2[1][$j], $matches2[0][$j], $out_html, $data, $params, $scode,$city);
                }
                // 执行替换
                $content = str_replace($matches[0][$i], $out_html, $content);
            }
                        ...
                        switch ($matches2[1][$j]) {
                            ...
                            case 'link':
                                ...
                                if ($target == 'tag') {
                                    ...
                                    $link = $this->parserLink($value['sort']->type, $value['sort']->urlname, 'list', $value['sort']->scode, $value['sort']->filename, '', '',$city);
                                    if ($url_rule_type == 3) {
    ...
    // 解析内容搜索结果标签
    public function parserSearchLabel($content,$city='')
    {
...
//        var_dump(111);die;
    ...
    // 解析列表标签
    protected function parserList($label, $search, $content, $data, $params, $key,$city='')
    {
        switch ($label) {
            case 'n':
                $content = str_replace($search, $this->adjustLabelData($params, $key) - 1, $content);
            ...
            case 'link':
                if ($data->outlink) { // 外链
                    ...
                    if ($data->type == 1) {
                        $content = str_replace($search, $this->parserLink($data->type, $data->urlname, 'about', $data->scode, $data->sortfilename, '', '',$city), $content);
                    } else {
                        $content = str_replace($search, $this->parserLink($data->type, $data->urlname, 'content', $data->scode, $data->sortfilename, $data->id, $data->filename,$city), $content);
                    }
                }
                break;
            case 'sortlink':
                $content = str_replace($search, $this->parserLink($data->type, $data->urlname, 'list', $data->scode, $data->sortfilename, '', '',$city), $content);
                break;
            case 'subsortlink':
                if ($data->subscode) {
                    $content = str_replace($search, $this->parserLink($data->type, $data->urlname, 'list', $data->subscode, $data->subfilename, '', '',$city), $content);
                } else {
    ...
    // 解析内容详情标签
    protected function parserContent($label, $search, $content, $data, $params, $sort,$city='')
    {
        switch ($label) {
            case 'link':
                if ($data->type == 1) {
            ...
            case 'precontent':
                ...
                if (! ! $pre = $this->model->getContentPre($sort->scode, $data->id)) {
                    $content = str_replace($search, '<a href="' . $this->parserLink($pre->type, $pre->urlname, 'content', $pre->scode, $pre->sortfilename, $pre->id, $pre->filename,$city) . '">' . $this->adjustLabelData($params, $pre->title) . '</a>', $content);
                } else {
            ...
            case 'prelink':
                if ($data->type != 2) // 非列表内容页不解析
                    break;
                if (! ! $pre = $this->model->getContentPre($sort->scode, $data->id)) {
                    $content = str_replace($search, $this->parserLink($pre->type, $pre->urlname, 'content', $pre->scode, $pre->sortfilename, $pre->id, $pre->filename,$city), $content);
                } else {
                    $content = str_replace($search, 'javascript:;', $content);
                }
                break;
            ...
            case 'nextcontent':
                ...
                if (! ! $next = $this->model->getContentNext($sort->scode, $data->id)) {
                    $content = str_replace($search, '<a href="' . $this->parserLink($next->type, $next->urlname, 'content', $next->scode, $next->sortfilename, $next->id, $next->filename,$city) . '">' . $this->adjustLabelData($params, $next->title) . '</a>', $content);
                } else {
                    if (isset($params['notext'])) {
                        $content = str_replace($search, $params['notext'], $content);
                    } else {
            ...
            case 'nextlink':
                if ($data->type != 2) // 非列表内容页不解析
                    break;
                if (! ! $next = $this->model->getContentNext($sort->scode, $data->id)) {
                    $content = str_replace($search, $this->parserLink($next->type, $next->urlname, 'content', $next->scode, $next->sortfilename, $next->id, $next->filename,$city), $content);
                } else {
                    $content = str_replace($search, 'javascript:;', $content);
                }
                break;
    ...
    // 解析生成内容链接
    public function parserLink($type, $urlname, $pagetype, $scode, $sortfilename, $id = '', $contentfilename = '', $city = '')
    {
        $url_break_char = $this->config('url_break_char') ?: '_';
        ...
        if($city){$city=$city.'/';}
        if ($type == 1 || $pagetype == 'about') {            $urlname = $urlname ?: 'about';
            if ($sortfilename) {
                $link = Url::home($city.$sortfilename);
            } else {
                $link = Url::home($city.$urlname . $url_break_char . $scode);
            }
        } else {
            ...
            if ($pagetype == 'list') {
                ...
                if ($url_rule_content_path) {
                    if ($contentfilename) {
                        $link = Url::home($city.$contentfilename, true);
                    } else {
                        $link = Url::home($city.$id, true);
                    }
                } else {
                    if ($sortfilename && $contentfilename) {
                        $link = Url::home($city.$sortfilename . '/' . $contentfilename, true);
                    } elseif ($sortfilename) {
                        $link = Url::home($city.$sortfilename . '/' . $id, true);
                    } elseif ($contentfilename) {
                        $link = Url::home($city.$urlname . $url_break_char . $scode . '/' . $contentfilename, true);
                    } else {
                        $link = Url::home($city.$urlname . $url_break_char . $scode . '/' . $id, true);
                    }
                }
            } else {
                $link = 'javascript:;';
