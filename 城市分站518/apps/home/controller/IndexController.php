...
class IndexController extends Controller
{
    ...
    public function _empty()
    {
        ...
        $allCity=$this->model->getAllCity();
        $P=P;

 

if(array_search($P, array_column($allCity, 'filename'))){
        ...
        foreach ($allCity as $key => $value) {
          $cityname=$value->filename;        
		  if(strpos($P,$cityname.'/')===0 ){   
		  $this->city =$cityname;
		  }	  
		} 
        ...
        $suffix = false;//是否包含.html
        $slash = false;
        $isCityHome=0; //是否城市首页JUSRUSH
        if (preg_match('/(.*)(_|\.)' . $url_rule_suffix . '$/', $path, $matchs)) {
            $path = $matchs[1];
            $suffix = true;
             //JUSRUSH  
            if($this->city){
            $path=str_replace($this->city.'/','',$path);    
              if($path==$this->city){
                 $isCityHome=2;
              } 
            } 
            if($matchs[0]=='city.html'){           
               $isCityHome=1;
               $this->getCityPage();
               exit;
             } 
...
if ($path_arr[1]=='sitemap.xml' && count($path_arr) == 2 && $this->config('sitemap_close')==0 && $this->config('sitemap_type')==0){
$sitemap = new SitemapController();
$sitemap->index($this->city);        
exit;
}
...
if($isCityHome==3){
 ...
 exit;
            ...
            if(SITE_DIR == '' ){
                ...
                if($matches[0] && SITE_DIR . '/' == $matches[0]) {
    ...
    private function getIndexPage()
    {
       ...
       // $city_mb=$this->city ? $this->city.'/' :'';
        ...
        $content = $this->parser->parserPositionLabel($content, - 1, '首页', SITE_INDEX_DIR . '/',$this->city); // CMS当前位置标签解析
        $content = $this->parser->parserSpecialPageSortLabel($content, 0, '', SITE_INDEX_DIR . '/',$this->city); // 解析分类标签
        $content = $this->parser->parserAfter($content,$this->city,1); // CMS公共标签后置解析
        $this->cache($content, true);
    }

    // 城市首页
    private function getCityPage()
    {
        ...
        $content = $this->parser->parserPositionLabel($content, - 1, '首页', SITE_INDEX_DIR . '/',$this->city); // CMS当前位置标签解析
        $content = $this->parser->parserSpecialPageSortLabel($content, 0, '', SITE_INDEX_DIR . '/',$this->city); // 解析分类标签
        $content = $this->parser->parserAfter($content,$this->city); // CMS公共标签后置解析
        $this->cache($content, true);
    }
    // 列表
    private function getListPage($sort)
    {
        ...
        if ($sort->listtpl) {
            ...
            $content = $this->parser->parserPositionLabel($content, $sort->scode,'','',$this->city); // CMS当前位置标签解析
            $content = $this->parser->parserSortLabel($content, $sort,$this->city); // CMS分类信息标签解析
            $content = $this->parser->parserListLabel($content, $sort->scode,$this->city); // CMS分类列表标签解析
            $content = $this->parser->parserAfter($content,$this->city); // CMS公共标签后置解析
        } else {
    ...
    private function getContentPage($data)
    {
        ...
        if (! ! $sort = $this->model->getSort($data->scode)) {
            if ($sort->contenttpl) {
                ...
                $content = $this->parser->parserPositionLabel($content, $sort->scode,'','',$this->city); // CMS当前位置标签解析
                $content = $this->parser->parserSortLabel($content, $sort,$this->city); // CMS分类信息标签解析
                $content = $this->parser->parserCurrentContentLabel($content, $sort, $data,$this->city); // CMS内容标签解析
